...
class AmModbusRtuClient {
public:
    AmModbusRtuClient();
    ~AmModbusRtuClient() = default;
    ...
    bool init_client(UART_HandleTypeDef *uart,
                    GPIO_TypeDef *enPort,
                    uint16_t enPin,
    ...
    bool asyn_read_holding(uint8_t slaveId,
                          uint16_t startAddr,
                          uint16_t regsCount,
                          uint16_t *outRegs);

    // 异步读取输入寄存器（FC=4）。
    bool asyn_read_input(uint8_t slaveId,
                        uint16_t startAddr,
                        uint16_t regsCount,
                        uint16_t *outRegs);

    // 异步写单保持寄存器（FC=6）。
    bool asyn_wirte_single(uint8_t slaveId,
                          uint16_t addr,
                          uint16_t value);

    // 异步写多保持寄存器（FC=16）。values 数组长度为 regsCount。
    bool asyn_wirte_multiple(uint8_t slaveId,
                            uint16_t startAddr,
                            uint16_t regsCount,
