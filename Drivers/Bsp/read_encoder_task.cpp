...
void encoder_update_data(uint8_t id, const uint32_t *data, bool state)
{
    ...
    {
    case 1:
        base_reg = REG_SENSOR1_POS_HI;
        break;
    case 2:
        base_reg = REG_SENSOR2_POS_HI;
        break;
    case 3:
        base_reg = REG_SENSOR3_POS_HI;
        break;
    default:
        return;
    }
...
void encoder_update_multi(const uint8_t id, const uint16_t count, const uint16_t value, const bool state)
{
    ...
    ModbusSafeWriteRegister(&encoder_forward_server, base_reg, count);
    ModbusSafeWriteRegister(&encoder_forward_server, base_reg + 1, value);
...
//     // encoder->init_();
//     bms_app.initMaster(&huart8,NULL,0,500,USART_HW);//huart8为主站接口
//     bool pending = false;
//     uint32_t notifyValue = 0;
...
//             bms_app.asyncReadHolding(0x04, 0x0000, 1, bms_results);
