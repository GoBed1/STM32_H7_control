#include "am_modbus_rtu.hpp"

AmModbusRtuClient::AmModbusRtuClient()
{
    // zero init handler_
}

bool AmModbusRtuClient::init_client(UART_HandleTypeDef *uart,
                           GPIO_TypeDef *enPort,
                           uint16_t enPin,
...
bool AmModbusRtuClient::asyn_read_holding(uint8_t slaveId,
                                 uint16_t startAddr,
                                 uint16_t regsCount,
                                 uint16_t *outRegs)
{
...
bool AmModbusRtuClient::asyn_read_input(uint8_t slaveId,
                               uint16_t startAddr,
                               uint16_t regsCount,
                               uint16_t *outRegs)
{
...
bool AmModbusRtuClient::asyn_wirte_single(uint8_t slaveId,
                                 uint16_t addr,
                                 uint16_t value)
{
...
bool AmModbusRtuClient::asyn_wirte_multiple(uint8_t slaveId,
                                   uint16_t startAddr,
                                   uint16_t regsCount,
